...
import { BasicItemService } from './items/basic-item.service';
...
export class BomService extends BaseService<Model<BomHeaderAttributes>> {
  getRawMaterialListByParentItemCode(req: Request<import("express-serve-static-core").ParamsDictionary, any, any, import("qs").ParsedQs, Record<string, any>>, parent_item_code: string, quantity_needed: number) {
  ...
  private async convertToResultDTO(
    ...
    const itemHeader = await this.BasicItemService.findItemByID(
      {} as Request,
  ...
  private async buildNestedDetails(
    ...
    for (const detail of details) {
      ...
      const itemDTO = await this.BasicItemService.findItemByID(
        {} as Request,
  ...
  private async calculateCostFromItem(
    ...
    const item = await this.BasicItemService.findItemByID({} as Request, itemPkid);
    if (!item) {
