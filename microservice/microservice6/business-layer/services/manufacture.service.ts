...
export class ManufactureService{
     ...
     */
    async getRawMaterialAvailability(req: Request, parent_item_code: string, quantity_needed: number): Promise<{ availability: boolean, status: string, item_code?: string }>{
        try{
            const bomList: BomListItemDTO[] = await this.bomService.getRawMaterialListByParentItemCode(req, parent_item_code, quantity_needed, new BomListItemDTO());
            if(bomList){
