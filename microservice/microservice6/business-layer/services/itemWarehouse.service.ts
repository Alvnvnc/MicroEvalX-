...
> {
  ...
  private async convertToResultDTO(
    ...
    if (itemWarehouse.item_pkid) {
      ...
      if (itemModel) {
        const BasicItemAttributes = itemModel.toJSON() as BasicItemAttributes;
        if (BasicItemAttributes.unit_pkid) {
          const unitModel = await this.unitRepository.findByID(
            {} as Request,
            BasicItemAttributes.unit_pkid,
          );
          if (unitModel) {
        ...
        itemData = {
          ...removeAuditColumnsFromItem(BasicItemAttributes),
          unit: unitData,
        };
      }
    }
